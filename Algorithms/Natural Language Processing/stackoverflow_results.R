...
review_clean <- function(docs){
  ...
  for(j in seq(docs)){
    ...
    docs[[j]] <- gsub("asp.net", "aspdotnet", docs[[j]])
  ...
  for(j in seq(docs)){
    ...
    docs[[j]] <- gsub("full stack", "fullstack", docs[[j]])
    docs[[j]] <- gsub("java script", "javascript", docs[[j]])
    docs[[j]] <- gsub("ror", "rubyonrails", docs[[j]])
    docs[[j]] <- gsub("amazon web services", "aws", docs[[j]])
    docs[[j]] <- gsub("big data", "bigdata", docs[[j]])
    docs[[j]] <- gsub("web application", "webapplications", docs[[j]])
    docs[[j]] <- gsub("spec", "specifications", docs[[j]])
    docs[[j]] <- gsub("up to date", "uptodate", docs[[j]])
    docs[[j]] <- gsub("angularjs", "angular", docs[[j]])
  }
